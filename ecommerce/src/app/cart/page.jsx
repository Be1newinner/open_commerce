    ...
    <div className="max-w-7xl my-20 mx-auto justify-between max-xl:w-[90%]">
      <div className="flex gap-10 max-xl:flex-wrap max-md:w-full max-md:h-full max-md:p-0 max-md:flex-wrap min-md:w-4/5 ">
        <div className="w-4/6 max-md:w-full lg:w-full md:w-full">
          {
            data.length !== 0 && (
              <div className="flex justify-between max-md:hidden p-4 bg-gray-200 rounded-t-md">
          ...
          {data.length === 0 ? (
            <div className="flex flex-col justify-center items-center h-64 p-6">
              <FaShoppingCart className="text-gray-500 mb-4" size={100} />
              <h2 className="text-xl font-semibold text-gray-700">No items found in your cart.</h2>
              <p className="text-sm text-gray-500 mt-2">Looks like your cart is empty. Start adding some items!</p>
              <Link href="/product" className="mt-4 px-6 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition duration-300">
                Start Shopping
              </Link>
            </div>
          ) : (
            data.map((item) => (
              <div key={item.sku} className="flex items-center max-md:hidden border border-gray-100 rounded-b-md">
              ...
              <div className="mt-8 max-md:px-10 md:px-4">
                ...
                <div className="flex gap-2 ">
                  <input
                    ...
                    id="couponCode"
                    name="couponCode"
                    className="border border-black h-12 w-2/5 max-md:w-full p-2 outline-none rounded-md"
                    placeholder="Enter Coupon Code"
                  />
                  <button className="h-full w-full bg-black text-white flex justify-center items-center py-3 rounded-md hover:bg-blue-700">
                    Apply
                  </button>
                </div>
              </div>
        ...
        {/* Summary Section */}
        {data.length > 0 && (
          <div className="w-2/6 shadow-2xl p-10 max-md:flex-wrap max-md:w-full border border-gray-200 rounded-md">
            <div>
            ...
            {/* Checkout Button */}
            <Link href="/checkout">
              <button className="bg-black text-white w-full h-12 mt-4 hover:bg-blue-700 rounded-md">
                Proceed to Checkout
              </button>
            </Link>
